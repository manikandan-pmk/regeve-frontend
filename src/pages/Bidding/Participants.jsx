...
export default function Participants() {
  ...
  const fetchParticipants = async () => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:1337/api/biddingparticipantforms/by-bidding/${documentId}`);
  ...
  // 🔹 VERIFY PARTICIPANT
  const verifyParticipant = async (targetDocumentId) => {
    try {
      setProcessingId(targetDocumentId);
      await fetch(`http://localhost:1337/api/biddingparticipantforms/${targetDocumentId}/verify`, {
        method: "PUT",
      });
      ...
      showToast("Error verifying participant. Please try again.", "error");
    } finally {
  ...
  const copyLink = () => {
    const publicLink = `${window.location.origin}/#/bid/${documentId}`;
  ...
  // 🔹 EXPORT CSV
  const exportCSV = () => {
    if (participants.length === 0) return showToast("No data to export", "error");
    const headers = ["S.No", "Name", "Draw ID", "Phone Number", "WhatsApp", "Age", "Gender", "Status", "Winner"];
  ...
  const displayBiddingId = participants[0]?.bidding?.biddingid || documentId?.substring(0, 6).toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F8FAFC] to-[#EFF6FF] font-sans text-slate-900 pb-20 relative overflow-hidden">
      ...
      <div className={`fixed top-6 right-6 z-50 transform transition-all duration-500 ease-out ${toast.show ? "translate-x-0 opacity-100" : "translate-x-20 opacity-0 pointer-events-none"}`}>
        <div className={`flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl border ${toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-emerald-100 text-emerald-700'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 pb-4">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
          <div className="animate-fade-in">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-slate-600">
              <span className="flex items-center gap-2 bg-white border border-slate-200 px-4 py-1.5 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-default">
                🎯 ID: <span className="text-indigo-600 font-black">{displayBiddingId}</span>
              </span>
              <span className="flex items-center gap-2 bg-white border border-slate-200 px-4 py-1.5 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-default">
          ...
          <div className="flex items-center gap-4">
            <button onClick={copyLink} className="cursor-pointer flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-xl shadow-sm hover:border-indigo-300 hover:text-indigo-700 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 active:scale-95">
            ...
            <button onClick={exportCSV} className="cursor-pointer flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-slate-900 to-slate-800 text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:from-slate-800 hover:to-slate-700 hover:-translate-y-0.5 transition-all duration-300 active:scale-95">
              <svg className="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
              Export CSV
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <StatCard title="Total Entries" count={totalCount} subtitle="All registered participants" icon="👥" color="text-indigo-600" bg="bg-indigo-50" />
          <StatCard title="Verified" count={verifiedCount} subtitle="Successfully verified" icon="✅" color="text-emerald-600" bg="bg-emerald-50" />
          <StatCard title="Winners" count={winnersCount} subtitle="Lucky drawn winners" icon="🏆" color="text-amber-500" bg="bg-amber-50" />
          <StatCard title="Pending" count={pendingCount} subtitle="Awaiting verification" icon="⏳" color="text-slate-500" bg="bg-slate-100" />
        </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/60">
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
            <div>
              <h2 className="text-2xl font-extrabold text-slate-800">Participant Roster</h2>
              <p className="text-sm font-medium text-slate-500 mt-1">
                Showing <span className="text-slate-900 font-bold">{filteredParticipants.length}</span> results
              </p>
            </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <tr>
                    <td colSpan="6" className="px-6 py-16 text-center text-slate-500">
                      <div className="flex justify-center mb-4">
                        <svg className="animate-spin h-10 w-10 text-indigo-600 drop-shadow-md" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                      </div>
                      <span className="font-bold text-lg animate-pulse">Loading participants...</span>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-20 text-center">
                      ...
                      <h3 className="text-xl font-extrabold text-slate-800">
                          {searchTerm ? "No match found" : "No participants found"}
                      </h3>
                      <p className="text-slate-500 font-medium mt-2">
                          {searchTerm ? `No results for "${searchTerm}"` : "Share the registration link to get started."}
                      </p>
                    </td>
                  </tr>
                ) : (
                  ...
                  filteredParticipants.map((p, index) => (
                    <tr key={p.documentId} className="hover:bg-indigo-50/40 transition-colors duration-300 group">
                      ...
                      <td className="px-6 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs font-semibold text-slate-500 mt-0.5">{p.phonenumber || "No Phone"}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5 whitespace-nowrap">
                      ...
                      <td className="px-6 py-5 whitespace-nowrap">
                        {p.is_verified ? (
                          <span className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-full text-xs font-extrabold uppercase tracking-wide border border-emerald-200 shadow-sm">
                            <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span> Verified
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-full text-xs font-extrabold uppercase tracking-wide border border-amber-200 shadow-sm">
                            <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping"></span> Pending
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-5 whitespace-nowrap text-center">
                        {p.is_winned_candidate ? (
                          <span className="inline-block text-2xl drop-shadow-md animate-bounce" title="Winner">🏆</span>
                        ) : (
                      ...
                      <td className="px-6 py-5 whitespace-nowrap text-right">
                        {!p.is_verified ? (
                          <button
                            onClick={() => verifyParticipant(p.documentId)}
          ...
          {!loading && participants.length > 0 && (
            <div className="p-5 border-t border-slate-100 bg-slate-50/80 text-sm text-slate-500 flex justify-between items-center font-medium">
              <p>Showing <span className="font-black text-slate-800">{filteredParticipants.length}</span> of <span className="font-black text-slate-800">{totalCount}</span> participants</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-md hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 ease-out flex flex-col group cursor-default">
      <div className="text-sm font-extrabold text-slate-500 mb-4 flex justify-between items-start uppercase tracking-wider">
        {title}
        <div className={`w-10 h-10 rounded-xl ${bg} flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition-transform duration-300`}>
          {icon}
        </div>
      </div>
      <div className="text-4xl font-black text-slate-900 mb-2 tracking-tight group-hover:text-indigo-900 transition-colors">{count}</div>
