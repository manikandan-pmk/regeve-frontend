...
const EventLuckyDraw = () => {
  ...
  const [eventLetters, setEventLetters] = useState(["", ""]);
  ...
  // Get event letters from event name
  const getEventLetters = (name) => {
    if (!name) return ["E", "V"];
    const letters = name
      .replace(/[^A-Za-z]/g, "")
      .toUpperCase()
  ...
  // Update display values when event letters change
 // Update display values when event letters change (ONLY on initial load or reset)
  useEffect(() => {
    // Only reset to 000 if we aren't spinning and haven't selected a winner yet
    if (eventLetters[0] && !isSpinning && !result) {
      setCurrentValues([eventLetters[0], eventLetters[1], "0", "0", "0"]);
      setFinalValues([eventLetters[0], eventLetters[1], "0", "0", "0"]);
    }
  }, [eventLetters[0], eventLetters[1], isSpinning, result]);
  ...
  const fetchEventData = async () => {
    ...
    try {
      ...
      if (eventData) {
        ...
        const companyName = eventData.admins?.[0]?.Company_Name || "";

        // Set event name display
        setEventName(companyName);

        // Extract first two letters
        const letters = getEventLetters(companyName);
        setEventLetters(letters);

        // Filter participants who are present and not already winners
        const eligible = (eventData.event_management_forms || []).filter(
          (participant) =>
            participant.IsPresent === true && participant.IsWinned !== true,
        );

        setParticipants(eligible);
        console.log("Eligible participants:", eligible);
      }
    } catch (error) {
  ...
  const startSpinning = () => {
    if (participants.length === 0) {
      alert(
        "No eligible participants available! Make sure participants are marked as present and not already winners.",
      );
      return;
    }
 ...
 // ✅ STOP SPINNING AND SELECT WINNER
  const stopSpinning = async () => {
    try {
      ...
      if (participants.length === 0) {
        setIsSpinning(false); // Stop if error
        alert("No eligible participants found for this event!");
        return;
      }

      // Call the lucky draw API while the wheel is still visually spinning!
      const response = await axios.get(
        `https://api.regeve.in/api/luckydraw/${documentId}`,
      );
      ...
      if (response.data.success) {
        ...
        // Get full Member ID
        const memberIdStr = String(winner.Member_ID || "000");

        // Take last 3 characters (002)
        const digits = memberIdStr.slice(-3).split("");

        // Build new values for the five boxes
        const newValues = [
          eventLetters[0] || "E", 
          eventLetters[1] || "V",
          digits[0],
        ...
        // NOW stop the animation and show the blast since we have the data
        setIsSpinning(false);
        ...
        setResult({
          message: "🎉 Event Winner Selected! 🎉",
          memberId: memberIdStr, 
          winner: winner,
          updateSuccess: true,
        });

        // Refresh participants list (this will no longer reset the boxes to 000!)
      ...
      if (errorMsg.includes("No eligible participants")) {
        alert(
          "No eligible participants found. Please check if participants are marked as present.",
        );
      } else {
                  ...
                  <div className="mt-6 p-6 bg-gradient-to-r from-yellow-500/10 to-purple-600/10 rounded-xl border border-yellow-400/20">
                    <div className="text-center">
                      ...
                      <p className="text-gray-200 text-lg">
                        You are the lucky winner of this event!
                      </p>
                    </div>
                  </div>
                </div>
      ...
      >
        ...
        >
          ...
          <p className="text-gray-300 text-lg font-light tracking-wide">
            {eventName || "Spin to select the winner!"}
          </p>
            ...
            <motion.div key={index} className="relative">
              ...
              >
                ...
                <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs text-gray-400 font-mono">
                  {index < 2 ? eventName?.charAt(index) || "E" : "0-9"}
                </div>
              </motion.div>
            </motion.div>
          ))}
        ...
        <motion.div variants={itemVariants} className="flex gap-4 mb-6">
          <motion.button
            ...
            disabled={isSpinning || loading || participants.length === 0}
        ...
        >
          <p className="font-light">Press START to begin the lucky draw!</p>
          {participants.length > 0 ? (
            <p className="text-xs mt-1 text-green-400">
              {participants.length} eligible participants loaded
            </p>
          ) : (
            !loading && (
              <p className="text-xs mt-1 text-red-400">
                No eligible participants found (Mark participants as present)
              </p>
            )
          )}
          {loading && (
